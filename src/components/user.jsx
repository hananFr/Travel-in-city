...
function User(props) {
    ...
    const onHandleBtn = (e) => {
        ...
        console.log(user);
        if (user.admin) {
            axios.put(`http://localhost:3900/api/users/admin/${user._id}`, admin).then(window.location.reload())

        }

        if (!user.admin) {
            axios.put(`http://localhost:3900/api/users/admin/${user._id}`, makeAdmin).then(window.location.reload())
        }        
    }
